...
constexpr double kTol = 1e-9;       // tolerance for closed-form comparisons
constexpr double kLooseTol = 1e-6;  // tolerance for finite-difference checks

// Representative x values in the open interval, plus endpoints.
const std::vector<double> kInteriorX = {-0.9, -0.5, -0.1,   0.0,
                                        0.1,  0.5,  0.7071, 0.9};
const std::vector<double> kAllX = {-1.0, -0.9, -0.5,   -0.1, 0.0,
                                   0.1,  0.5,  0.7071, 0.9,  1.0};
...
TEST(LegendrePolynomial, P0IsOne) {
  for (double x : kAllX) {
    EXPECT_NEAR(LegendrePolynomial(0, x), 1.0, kTol) << "x=" << x;
  }
}

TEST(LegendrePolynomial, P1IsX) {
  for (double x : kAllX) {
    EXPECT_NEAR(LegendrePolynomial(1, x), x, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomial, P2ClosedForm) {
  for (double x : kAllX) {
    ...
    EXPECT_NEAR(LegendrePolynomial(2, x), expected, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomial, P3ClosedForm) {
  for (double x : kAllX) {
    ...
    EXPECT_NEAR(LegendrePolynomial(3, x), expected, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomial, P4ClosedForm) {
  for (double x : kAllX) {
    ...
    EXPECT_NEAR(LegendrePolynomial(4, x), expected, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomial, P5ClosedForm) {
  for (double x : kAllX) {
    ...
    EXPECT_NEAR(LegendrePolynomial(5, x), expected, kTol) << "x=" << x;
  }
}

TEST(LegendrePolynomial, EndpointValues) {
  for (int n = 0; n <= 10; ++n) {
    EXPECT_NEAR(LegendrePolynomial(n, 1.0), 1.0, kTol) << "n=" << n;
    double expected_minus_one = (n % 2 == 0) ? 1.0 : -1.0;
    EXPECT_NEAR(LegendrePolynomial(n, -1.0), expected_minus_one, kTol)
        << "n=" << n;
  }
}
...
TEST(LegendrePolynomial, BonnetRecurrence) {
  for (double x : kAllX) {
    for (int n = 1; n <= 9; ++n) {
      ...
      EXPECT_NEAR(lhs, rhs, 1e-8) << "n=" << n << " x=" << x;
    }
  }
}
...
TEST(LegendrePolynomial, ParitySymmetry) {
  for (double x : kAllX) {
    for (int n = 0; n <= 8; ++n) {
      ...
      EXPECT_NEAR(LegendrePolynomial(n, -x), sign * LegendrePolynomial(n, x),
                  kTol)
          << "n=" << n << " x=" << x;
    }
  }
...
TEST(LegendrePolynomialDerivative, D0IsZero) {
  for (double x : kAllX) {
    EXPECT_NEAR(LegendrePolynomialDerivative(0, x), 0.0, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomialDerivative, D1IsOne) {
  for (double x : kInteriorX) {
    EXPECT_NEAR(LegendrePolynomialDerivative(1, x), 1.0, kTol) << "x=" << x;
  }
}

TEST(LegendrePolynomialDerivative, D2ClosedForm) {
  for (double x : kInteriorX) {
    EXPECT_NEAR(LegendrePolynomialDerivative(2, x), 3.0 * x, kTol) << "x=" << x;
  }
}
...
TEST(LegendrePolynomialDerivative, D3ClosedForm) {
  for (double x : kInteriorX) {
    ...
    EXPECT_NEAR(LegendrePolynomialDerivative(3, x), expected, kTol)
        << "x=" << x;
  }
}
...
TEST(LegendrePolynomialDerivative, D4ClosedForm) {
  for (double x : kInteriorX) {
    ...
    EXPECT_NEAR(LegendrePolynomialDerivative(4, x), expected, kTol)
        << "x=" << x;
  }
}
...
TEST(LegendrePolynomialDerivative, MatchesFiniteDifference) {
  ...
  for (int n = 0; n <= 8; ++n) {
    for (double x : kInteriorX) {
      ...
      EXPECT_NEAR(analytic, numerical, kLooseTol) << "n=" << n << " x=" << x;
    }
  }
}
...
TEST(LegendrePolynomialDerivative, SatisfiesLegendreODE) {
  ...
  for (int n = 0; n <= 6; ++n) {
    for (double x : kInteriorX) {
      ...
      double d2 = (LegendrePolynomialDerivative(n, x + h) -
                   LegendrePolynomialDerivative(n, x - h)) /
                  (2.0 * h);
      double lhs = (1.0 - x * x) * d2 - 2.0 * x * d1 +
                   n * (n + 1) * LegendrePolynomial(n, x);
      EXPECT_NEAR(lhs, 0.0, 1e-3) << "n=" << n << " x=" << x;
    }
  }
}

// Endpoint derivative values: P_n'(1) = n(n+1)/2, P_n'(-1) = (-1)^(n+1) *
// n(n+1)/2. The current implementation only special-cases n==0 at the
// endpoints; for n>=1 the general formula divides by (1-x^2)=0, so this
// test is expected to fail (inf/nan) until endpoint handling is added for
// all n, not just n==0.
TEST(LegendrePolynomialDerivative, EndpointDerivativeAtPlusOne) {
  for (int n = 0; n <= 10; ++n) {
    double expected = n * (n + 1) / 2.0;
    EXPECT_NEAR(LegendrePolynomialDerivative(n, 1.0), expected, kTol)
        << "n=" << n;
  }
}
