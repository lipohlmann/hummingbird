...
[[nodiscard]]
static te_type te_fac(
    te_type val) noexcept { /* simplest version of factorial */
...
[[nodiscard]]
static te_type te_average(te_type val1, te_type val2, te_type val3,
                          te_type val4, te_type val5, te_type val6,
                          te_type val7, te_type val8, te_type val9,
                          te_type val10, te_type val11, te_type val12,
                          te_type val13, te_type val14, te_type val15,
                          te_type val16, te_type val17, te_type val18,
                          te_type val19, te_type val20, te_type val21,
                          te_type val22, te_type val23, te_type val24) {
...
static te_type te_ncr(te_type val1, te_type val2) noexcept {
  ...
  if (te_parser::te_exceeds_uint32(val1) ||
      te_parser::te_exceeds_uint32(val2)) {
    return std::numeric_limits<te_type>::infinity();
  }
...
static te_type te_right_shift(te_type val1, te_type val2) {
  ...
  if (std::floor(val2) != val2) {
    throw std::runtime_error(
        "Additive expression of right shift (>>) operation must be an "
        "integer.");
  }
  if (val1 < 0) {
    throw std::runtime_error(
...
[[nodiscard]]
static te_type te_and_variadic(te_type val1, te_type val2, te_type val3,
                               te_type val4, te_type val5, te_type val6,
                               te_type val7, te_type val8, te_type val9,
                               te_type val10, te_type val11, te_type val12,
                               te_type val13, te_type val14, te_type val15,
...
[[nodiscard]]
static te_type te_or_variadic(te_type val1, te_type val2, te_type val3,
                              te_type val4, te_type val5, te_type val6,
                              te_type val7, te_type val8, te_type val9,
                              te_type val10, te_type val11, te_type val12,
                              te_type val13, te_type val14, te_type val15,
...
[[nodiscard]]
static te_type te_ifs(te_type if1, te_type if1True, te_type if2,
                      te_type if2True, te_type if3, te_type if3True,
                      te_type if4, te_type if4True, te_type if5,
                      te_type if5True, te_type if6, te_type if6True,
                      te_type if7, te_type if7True, te_type if8,
                      te_type if8True, te_type if9, te_type if9True,
                      te_type if10, te_type if10True, te_type if11,
...
static te_type te_numbervalue(std::span<const te_arg> args) {
  ...
  if (const auto* const num = std::get_if<te_type>(args.data());
      num != nullptr) {
    return *num;
  }
  ...
  const auto getSeparatorArg = [&args](const size_t index,
                                       const char defaultSep) -> char {
    if (index >= args.size()) {
      return defaultSep;
    }
    const auto* const sep = std::get_if<std::string_view>(&args[index]);
    return (sep == nullptr || sep->empty()) ? static_cast<char>(0)
                                            : sep->front();
  };

  const char decimalSep = getSeparatorArg(1, '.');
  // the default group separator is whichever of '.' and ',' the decimal one
  // isn't
  ...
  // group separator (the character list is spelled out to be locale
  // independent)
  constexpr std::string_view WHITESPACE{" \t\n\r"};
  std::string_view text{std::get<std::string_view>(args[0])};
  while (!text.empty() &&
         WHITESPACE.find(text.front()) != std::string_view::npos) {
  ...
  while (!text.empty() && (text.back() == '%' || WHITESPACE.find(text.back()) !=
  ...
  if (decimalPos != std::string_view::npos &&
      text.find_first_of(decimalSep, decimalPos + 1) !=
          std::string_view::npos) {
    return te_parser::te_nan;
  }
  ...
  // strtod() needs a null-terminated buffer, so this is where the copy has to
  // happen
...
#ifdef TE_FLOAT
  const auto result =
      static_cast<te_type>(std::strtof(normalized.c_str(), &nEnd));
#elif defined(TE_LONG_DOUBLE)
  const auto result =
      static_cast<te_type>(std::strtold(normalized.c_str(), &nEnd));
#else
  const auto result =
      static_cast<te_type>(std::strtod(normalized.c_str(), &nEnd));
#endif
  // unlike the lexer, the whole string has to convert (i.e., stopped at the
  // null terminator)
...
const std::set<te_variable> te_parser::m_functions = {  // NOLINT
    ...
    {"clamp",
     static_cast<te_fun3>([](const te_type num, const te_type start,
                             const te_type end)  // NOLINT
                          {
                            return (start <= end)
    ...
    {"numbervalue", static_cast<te_arg_fun>(te_builtins::te_numbervalue),
     TE_PURE},
...
#endif
      // Normally strtod() stops at the list separator and the lexer reads it
      // next. It only consumes one if the locale's decimal point is that
      // separator, which means the locale and the parser
      // (get_decimal_separator()) disagree. Raise a syntax error in that case.
      const std::string_view consumed{
          theState->m_next, static_cast<size_t>(nEnd - theState->m_next)};
      theState->m_next = nEnd;
      theState->m_type =
          (consumed.find(get_list_separator()) == std::string_view::npos)
      ...
      if (is_letter(*theState->m_next) || (*theState->m_next == '_')) {
        ...
        if (tok == '"') {
          ...
          if (*theState->m_next != '"') {
            ...
            theState->m_value = std::string_view{
                strStart, static_cast<std::string_view::size_type>(
...
te_expr* te_parser::base(state* theState) {
  ...
  // where nesting is capped. Without it, deeply nested input could overflow the
  // stack.
...
te_expr* te_parser::base_impl(state* theState) {
  ...
  } else if (is_arg_function(theState->m_value) ||
             is_arg_closure(theState->m_value)) {
      ...
      if (theState->m_type == state::token_type::TOK_CLOSE) {
        ...
        while (true) {
          if (theState->m_type == state::token_type::TOK_STRING) {
            ret->m_parameters.push_back(
                new_expr(TE_DEFAULT, theState->m_value));
            next_token(theState);
          } else {
...
te_expr* te_parser::factor(te_parser::state* theState) {
  ...
  if (theState->m_appliedUnary && ret->m_type == TE_PURE &&
  ...
  while (theState->m_type == state::token_type::TOK_INFIX &&
    ...
    if (slotOwner != nullptr) {
      insert =
          new_expr(TE_PURE, t, {slotOwner->m_parameters[slotIndex], exponent});
...
te_type te_parser::evaluate(
    const std::string_view
        expression)  // NOLINT(-readability-identifier-naming)
{
