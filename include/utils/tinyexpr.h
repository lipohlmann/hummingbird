...
/// @brief Define this to use @c float instead of @c double for the parser's
/// data type.
#ifdef TE_FLOAT
...
///     Review `args.size()` and each argument's type, returning
///     te_parser::te_nan if invalid.
/// @warning The span is only valid for the duration of the call.
using te_arg_fun = te_type (*)(std::span<const te_arg>);
/// @brief Context version of te_arg_fun (te_variable passes a client's te_expr
/// first).
/// @warning The span is only valid for the duration of the call.
...
// te_arg_confun matches the above specializations, but is not a fixed-arity
// closure.
template <>
...
constexpr bool te_is_function_v =
    ...
    !te_is_string_v<T> && !te_is_arg_function_v<T> && !te_is_arg_closure_v<T>;
...
using te_variant_type = std::variant<
    te_type, const te_type*,  // indices 0-1
    te_fun0, te_fun1, te_fun2, te_fun3, te_fun4, te_fun5, te_fun6, te_fun7,
    te_fun8, te_fun9, te_fun10, te_fun11, te_fun12, te_fun13, te_fun14,
    te_fun15, te_fun16, te_fun17, te_fun18, te_fun19, te_fun20, te_fun21,
    te_fun22, te_fun23, te_fun24, te_confun0, te_confun1, te_confun2,
    te_confun3, te_confun4, te_confun5, te_confun6, te_confun7, te_confun8,
    te_confun9, te_confun10, te_confun11, te_confun12, te_confun13, te_confun14,
    te_confun15, te_confun16, te_confun17, te_confun18, te_confun19,
    te_confun20, te_confun21, te_confun22, te_confun23, te_confun24,
...
/// @internal Note that because this is a bitmask, don't declare it as an enum
/// class,
///     just a C-style enum.
enum te_variable_flags {
  ...
  TE_PURE = (1 << 0),
  /// @brief Function that can take 1-24 argument (unused arguments are set to
  /// NaN).
  TE_VARIADIC = (1 << 1)
};
...
class te_variable {
 public:
  ...
  name_type m_name;
  /// @brief The te_type constant, te_type pointer, or function to bind the name
  /// to.
  te_variant_type m_value;
  /// @brief The type that m_value represents.
  te_variable_flags m_type{TE_DEFAULT};
  /// If @c m_value is a function pointer of type `te_confun0`-`te_confun24`,
  /// then this is passed to that function when called. This is useful for
  /// passing an object which manages additional data to your functions.
...
class te_parser {
 public:
  ...
  /// @returns @c true if the parser's internal type can hold `uint32_t` without
  /// truncation.
  [[nodiscard]]
  ...
  /// @returns @c true if the parser's internal type can hold `uint64_t` without
  /// truncation.
  [[nodiscard]]
  ...
  /// @returns The largest integer value that the parser can handle without
  /// truncation.
  [[nodiscard]]
  static te_type get_max_integer() {
#ifdef TE_FLOAT
    const te_type maxBit =
        std::ldexp(static_cast<te_type>(1), FLT_MANT_DIG - 1);
#elif defined(TE_LONG_DOUBLE)
    const te_type maxBit =
        std::ldexp(static_cast<te_type>(1), LDBL_MANT_DIG - 1);
#else
    const te_type maxBit =
        std::ldexp(static_cast<te_type>(1), DBL_MANT_DIG - 1);
#endif
  ...
  /** @brief Parses the input @c expression.
      ...
      @throws std::runtime_error Throws an exception in the case of arithmetic
     overflows (e.g., `1 << 64` would cause an overflow).*/
  bool compile(std::string_view expression);
  /** @brief Evaluates expression passed to compile() previously and returns its
     result.
  ...
  /// @returns The last call to evaluate()'s result (which will be NaN on
  /// error).
  [[nodiscard]]
  ...
  /// @returns The zero-based index into the last parsed expression where the
  /// parse failed,
  ...
  /// @note Valid variable and function names must begin with a letter from a-z
  /// (A-Z),
  ///     followed by additional English letters, numbers, periods, or
  ///     underscores.
  /// @throws std::runtime_error Throws an exception if an illegal character is
  /// found
  ...
  /// @note Prefer using set_variables_and_functions() as it will be more
  /// optimal
  ...
  void remove_variable_or_function(te_variable::name_type var) {
    const auto foundVar = m_customFuncsAndVars.find(te_variable{
        std::move(var), static_cast<te_type>(0.0), TE_DEFAULT, nullptr});
  ...
  /// @brief Removes any custom variables and functions that weren't used in the
  /// last compilation.
  /// @details This can be useful if the parser is preloaded with a large number
  /// of
  ///     variables and functions that needs to be pruned after the first
  ///     expression is parsed.
  /// @warning After calling this, any custom variables and functions that
  /// weren't found
  ///     in the previously parsed expression will no longer be available.
  void remove_unused_variables_and_functions() {
    for (auto funcIter = m_customFuncsAndVars.cbegin();
         funcIter != m_customFuncsAndVars.cend();
         /* in loop*/) {
      funcIter = (is_function_used(funcIter->m_name) ||
                  is_variable_used(funcIter->m_name))
                     ? ++funcIter
  ...
  /** @brief Sets a custom function to resolve unknown symbols in an expression.
      ...
      @param keepResolvedVariables @c true to cache any resolved variables into
     the parser. This means that they will not need to be resolved again on
     subsequent calls to evaluate().\n Pass @c false to this if you wish to
     re-resolve any previously resolved variables on later evaluations. This can
     be useful for when a resolved variable's value is volatile and needs to be
  ...
  void set_decimal_separator(const char sep) {
    if (sep != ',' && sep != '.') {
      throw std::runtime_error(
          "Decimal separator must be either a '.' or ','.");
    }
  ...
  /// @note If the constant variable hasn't been added yet (via
  ...
  te_type get_constant(const std::string_view name) const {
    auto cvar = find_variable_or_function(name);
    if (cvar == get_variables_and_functions().cend() ||
        !is_constant(cvar->m_value)) {
      return te_nan;
    }
    if (const auto* const val = std::get_if<te_type>(&cvar->m_value);
        val != nullptr) {
      return *val;
    }
...
#ifndef TE_NO_BOOKKEEPING
  /// @returns @c true if @c name is a function that had been used in the last
  /// parsed formula.
  ...
  /// @returns @c true if @c name is a variable that had been used in the last
  /// parsed formula.
  ...
  /// @returns Information about how the parser is configured, its capabilities,
  /// etc.
  [[nodiscard]]
 ...
 private:
  ...
  static void validate_name(const te_variable& var) {
    ...
    if (!is_letter(var.m_name[0]) && var.m_name[0] != '_') {
      throw std::runtime_error(
          std::string(
              "Variable name must begin with a letter from a-z or _: ") +
          var.m_name);
    }
    const auto varCharPos = std::find_if(
        var.m_name.cbegin(), var.m_name.cend(),
  ...
  constexpr static bool is_name_char_valid(const char chr) noexcept {
    return (is_letter(chr) || (chr >= '0' && chr <= '9') || (chr == '_') ||
            (chr == '.'));
  }

  /// @returns The position of @p chr at or after @p startIndex, skipping quoted
  /// string
  ...
  /// @note Quote state is tracked from the start of @p str, so this stays
  /// correct
  ...
  /// @returns An iterator to the custom variable or function with the given @c
  /// name,
  ///     or end of get_variables_and_functions() if not found.
  /// @param name The name of the function or variable to search for.
  [[nodiscard]]
  std::set<te_variable>::iterator find_variable_or_function(
      const std::string_view name) {
    if (name.empty()) {
      return m_customFuncsAndVars.end();
    }

    return m_customFuncsAndVars.find(te_variable{te_variable::name_type{name},
                                                 static_cast<te_type>(0.0),
                                                 TE_DEFAULT, nullptr});
  }
  ...
  /// @param name The name of the function or variable to search for.
  [[nodiscard]]
  std::set<te_variable>::const_iterator find_variable_or_function(
    ...
    return std::visit(
        [](const auto& var0) -> size_t {
          ...
          if constexpr (te_is_constant_v<T> || te_is_variable_v<T> ||
                        te_is_string_v<T> || te_is_arg_function_v<T> ||
                        te_is_arg_closure_v<T>) {
            return 0;
...
#undef TE_DEF_CLOSURE
  ...
  /// @returns @c true for any context function, including the ones taking
  /// te_arg.
  [[nodiscard]]
  ...
  static size_t get_parameter_count(const te_expr* texp) {
    ...
    return (texp->m_parameters.size() > ctxSlot)
               ? texp->m_parameters.size() - ctxSlot
               : 0;
  }
  ...
  [[nodiscard]]
  static te_expr* new_expr(const te_variable_flags type,
  ...
  [[nodiscard]]
  te_expr* te_compile(std::string_view expression,
  ...
  static auto find_builtin(const std::string_view name) {
    return m_functions.find(te_variable{te_variable::name_type{name},
                                        static_cast<te_type>(0.0), TE_DEFAULT,
                                        nullptr});
  }
